...
app.get('/api/auth/local-token', rateLimit({ windowMs: 60000, maxRequests: 30 }), (req, res) => {
  const allowLanLocalToken = process.env.ALLOW_LAN_LOCAL_TOKEN !== 'false';
  const rawIp = req.ip || req.socket.remoteAddress || '';
  const ip = rawIp.replace(/^::ffff:/, '');
  const isLoopback = ip === '127.0.0.1' || ip === '::1';
  const isPrivateLan = /^10\./.test(ip) || /^192\.168\./.test(ip) || /^172\.(1[6-9]|2\d|3[0-1])\./.test(ip);

  if (!isLoopback && !(allowLanLocalToken && isPrivateLan)) {
